...
class AgentProductService
{
    ...
    public function update($data, $agent, $agentProduct){
        $response = [];
        try {
            DB::beginTransaction();
            $product = Product::query()->where('id',$agentProduct->product_id)->first();
            ...
            if ($product->quantity >= $data['quantity']) {
                ...
                DB::commit();
                $response = ['key' => 'success', 'message' => 'Muvaffaqiyatli yangilandi'];
            } else {
                DB::rollBack();
                $response = ['key' => 'error', 'message' => 'Ombordagi mahsulot soni yetarli emas!'];
            }
